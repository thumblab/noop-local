...
class DynamodbContainer extends ResourceContainer {
  ...
  start (done) {
    super.start((err) => {
      ...
      setupContainer.cmd = ['dynamodb', 'create-table', '--endpoint-url', `http://${this.name}:8000`, '--region', 'us-east-1', '--table-name', this.name, '--attribute-definitions', attributeDefinitions, '--key-schema', keySchema, '--provisioned-throughput', 'ReadCapacityUnits=5,WriteCapacityUnits=5']
