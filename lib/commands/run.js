...
module.exports = (argv) => {
  ...
  async.auto({
    ...
    devServer: ['app', 'envOverrides', (results, done) => {
      done(null, new DevServer(results.app, port, results.envOverrides))
    }],
...
function parseEnvOverrides (statements, files, rootDir, done) {
  ...
  function parseOverride(line) {
    ...
    const match = /^([a-zA-Z0-9-]+)\.([a-zA-Z0-9-]+)=([a-zA-Z0-9-]+)$/.exec(line)
    if (!match) return console.log(chalk.yellow(`Invalid env override: '${line}'`))
    if (!overrides[match[1]]) overrides[match[1]] = {}
    overrides[match[1]][match[2]] = match[3]
  }
  if (typeof statements === 'string') statements = [statements]
  if (!files) {
    files = []
