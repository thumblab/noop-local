...
module.exports = (argv) => {
  ...
  async.auto({
    ...
    browser: ['inspector', 'start', (results, done) => {
      exec(`open http://localhost:${results.inspector.port}`, done)
    }],
    wait: ['browser', (results, done) => {
      console.log(chalk.cyan(`The noop inspector is running on http://localhost:${results.inspector.port}`))
