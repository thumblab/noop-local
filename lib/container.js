...
class Container extends EventEmitter {
  ...
  start (done) {
    ...
    async.auto({
      ...
      stats: ['container', (results, done) => {
        if (this.type === 'component') {
          this.handleStats(done)
        } else {
          done()
        }
      }],
      start: ['network', 'output', 'stats', (results, done) => {
      ...
      stream.on('data', (data) => {
        ...
        if (data) {
          ...
          if (this.devServer.verbose || this.outputLogs) {
            ...
            const spaces = ' '.repeat(this.friendlyName.length < 16 ? 16 - this.friendlyName.length : 0)
  ...
  log (data) {
    ...
    if (this.logs.length >= this.numOfLogs) {
      this.logs.splice(0, this.logs.length - this.numOfLogs - 1)
    }
    this.logs.push(log)
    this.emit('log', log)
  }
