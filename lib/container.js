...
class Container {
  constructor (devServer, friendlyName, type) {
    ...
    this.specials = []
    this.cmd = null
    this.daemon = (type === 'job') ? false : true
  }
  ...
  start (done) {
    ...
    if (this.component) {
      const directives = this.component.directives.filter((directive) => {
    ...
    async.auto({
      ...
      volumes: (done) => {
        if ('getVolumes' in this) {
          this.getVolumes(done)
        } else {
          done(null)
        }
      },
      container: ['volumes', 'inspectImage', 'removeExisting', (results, done) => {
        ...
        if (this.type === 'router') {
          ...
          if (this.devServer.http === true) {
            opts.HostConfig.PortBindings['80/tcp'] = [{HostPort: this.devServer.port.toString()}]
          } else {
            opts.HostConfig.PortBindings['443/tcp'] = [{HostPort: this.devServer.port.toString()}]
          }
  ...
  handleOutput (done) {
    if (!this.devServer.verbose) {
      if (this.type === 'resource') return done()
      if (this.type === 'job') return done()
    }
    const opts = {
      stream: true,
      stdout: true,
    ...
    this.container.attach(opts, (err, stream) => {
      ...
      if (this.type === 'router') {
        prefix = chalk.magenta(this.friendlyName.substr(0, 15))
      } else {
        prefix = chalk.cyan(this.friendlyName.substr(0, 15))
      }
      stream.on('data', (data) => {
