...
class Inspector {
  ...
  init () {
    ...
    this.app.ws('/api/components/:component/logs', (ws, req) => {
      const container = this.server.componentContainers.find((container) => {
        return container.component.name === req.params.component
      })
    ...
    this.app.get('/api/components/:component/performance', (req, res) => {
      ...
      component.container.stats({stream:true}, (err, stream) => {
        ...
        stream.on('data', (data) => res.write(`data: ${data.toString()}\n\n`))
    ...
    this.app.ws('/api/components/:component/terminal', (ws, req) => {
      ...
      component.container.exec(execOpts, (err, exec) => {
        ...
        exec.start({stdin: true}, (err, dockerStream) => {
          ...
          dockerStream.on('end', () => {
            ws.send(JSON.stringify({s: 'system', d: new Buffer('\u001b[31;1mConnection to container lost!\u001b[0m').toString('base64')}), () => {})
            ws.close()
          })
          ws.on('message', (msg) => {
            let payload
    ...
    this.app.get('/api/resources/:resource', (req, res) => {
      ...
      res.json({
        id: container.resource.name,
        type: container.resource.type,
        parameters: container.resource.params,
        components,
        container: {
          image: container.getImage(),
          name: container.name
    ...
    this.app.post('/api/events/public', (req, res) => {
      const params = {
        uri: `http://localhost:${this.server.routerInterface.port}/_noop/events/public`,
        method: 'POST',
        json: true,
        body: req.body
      }
      request.post(params, (err, serverResponse) => {
        if (err) {
          console.log(err)
          return res.send(err)
        }
        res.json(serverResponse.body)
      })
    })
