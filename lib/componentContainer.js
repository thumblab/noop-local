...
class ComponentContainer extends Container {
  ...
  getEnv() {
    ...
    Object.keys(this.inspectorEnvOverrides).forEach((key) => env[key] = this.inspectorEnvOverrides[key])
  ...
  getEnvInheritance() {
    ...
    Object.keys(this.component.env).forEach((key) => {
      ...
      variables[key].unshift(resolve('noopfile', this.component.env[key].default))
    })
    //runtime-wildcard
    Object.keys(this.devServer.envOverrides['_all']).forEach((key) => {
      if (!variables[key]) variables[key] = []
