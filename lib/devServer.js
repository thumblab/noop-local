...
class DevServer {
  ...
  reload () {
    if (!this.reloading) {
      ...
      console.log('Reloading local Noop Dev Server...')
      this.reloading = true
      async.auto({
        ...
        reloadComplete: ['start', (results, done) => {
          this.reloading = false
          // console.log(`Reload completed for local Noop Dev Server in ${(Date.now() - reloadStart) / 1000}s`)
          console.log('Reload completed for local Noop Dev Server')
          done()
        }]
      }, (err) => {
