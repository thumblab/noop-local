...
class DevServer {
  ...
  start (done) {
    async.auto({
      ...
      postdeployDelay: ['startComponents', (results, done) => {
        setTimeout(() => done(), 1000)
      }],
      postdeployTasks: ['postdeployDelay', (results, done) => {
        const containers = this.componentContainers.filter((componentContainer) => {
          return (componentContainer.component.type === 'task' && componentContainer.component.lifecycles.includes('postdeploy'))
        })
        async.eachSeries(containers, (container, done) => {
          console.log(`Starting postdeploy task '${container.friendlyName}'`)
          container.once('end', (statusCode) => {
            if (statusCode) return done(new Error(`Error running postdeploy task '${container.friendlyName}'`))
            console.log( `Predeploy task '${container.friendlyName}' completed successfully`)
            done()
          })
          container.start()
      ...
      if (err) {
        ...
        fs.readFile(cacheFile, (err, data) => {
          ...
          try {
            ...
            this.certificateChain = new Buffer.from(json.chain, 'base64').toString()
            this.certificateKey = new Buffer.from(json.key, 'base64').toString()
            done()
          } catch (err) {
            done(err)
          }
