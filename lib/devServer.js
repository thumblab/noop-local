...
class DevServer {
  ...
  start (done) {
    async.auto({
      ...
      startResources: ['build', 'network', (results, done) => {
        async.each(this.resourceContainers, (container, done) => container.start(done), done)
      }],
      buildRouter: ['startResources', (results, done) => this.router.build(done)],
      startRouter: ['network', 'buildRouter', (results, done) => {
        this.router.start(done)
      }],
      startComponents: ['startResources', 'startRouter', (results, done) => {
