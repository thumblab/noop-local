...
class DevServer {
  ...
  handleCron (container) {
    console.log(`${moment().format('LT')} - Running task '${container.friendlyName}' on cron schedule`)
    if (activeTasks.has(container.name)) {
      console.log(chalk.red(`Task '${container.friendlyName}' is already running`))
    } else {
      activeTasks.add(container.name)
      container.once('end', (statusCode) => {
        if (statusCode) {
          console.log(chalk.red(`Error running cron task '${container.friendlyName}'`))
        }
        console.log(`Cron task '${container.friendlyName}' completed successfully`)
        activeTasks.delete(container.name)
      })
      container.start()
    }
  }
  ...
  manualTaskRun (container) {
    if (activeTasks.has(container.name)) {
      ...
      container.once('end', (statusCode) => {
        if (statusCode) {
          console.log(chalk.red(`Error running task '${container.friendlyName}'`))
        }
        console.log(`Task '${container.friendlyName}' completed successfully`)
        activeTasks.delete(container.name)
      })
      container.start()
    }
