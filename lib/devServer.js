...
class DevServer {
  ...
  handleCron (container) {
    ...
    console.log(`Running task '${container.friendlyName}' on cron schedule (${time})`)
    container.once('end', (statusCode) => {
      if (statusCode) {
        console.log(chalk.red(`Error running cron task '${container.friendlyName}' (${time})`))
      }
      console.log(`Cron task '${container.friendlyName}' completed successfully (${time})`)
    })
    container.start()
  }
  ...
  manualTaskRun (container) {
    const time = moment().format('LTS')
    console.log(`Manually Running task '${container.friendlyName}' (${time})`)
    container.once('end', (statusCode) => {
      if (statusCode) {
        console.log(chalk.red(`Error running task '${container.friendlyName}' (${time})`))
      }
      console.log(`Task '${container.friendlyName}' completed successfully (${time})`)
    })
    container.start()
  }
