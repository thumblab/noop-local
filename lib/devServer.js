...
class DevServer {
  constructor (app, port, envOverrides, opts) {
    ...
    this.componentNames = opts.componentNames
    this.resourceNames = opts.resourceNames
  ...
  loadApp () {
    ...
    this.componentContainers = Object.keys(this.app.components)
      .filter((componentName) => {
        return this.componentNames.includes(componentName) || !this.componentNames.length
      })
      .map((componentName) => {
    ...
    this.resourceContainers = Object.keys(this.app.resources)
      .filter((resourceName) => {
        return this.resourceNames.includes(resourceName) || !this.resourceNames.length
      })
      .map((resourceName) => {
  ...
  reloadComponent (componentName) {
    if (!this.reloading && this.autoReload && (this.componentNames.includes(componentName) || !this.componentNames.length)) {
